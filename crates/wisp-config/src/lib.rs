...
impl PartialConfig {
    ...
    fn from_environment(env_overrides: &BTreeMap<String, String>) -> Result<Self, ConfigError> {
        ...
        if let Some(value) = env_overrides.get("WISP_EMBERS_SOCKET") {
            // Store the raw value so an empty override still merges over a file
            // value (merge_option ignores None); emptiness is normalized to None
            // later in `resolve`.
    ...
    fn resolve(self) -> Result<ResolvedConfig, ConfigError> {
        ...
        config.embers.socket_path = self.embers.socket_path.and_then(non_empty_socket_path);
...
/// Normalize a configured embers socket path: an empty value (e.g.
/// `WISP_EMBERS_SOCKET=` or `socket_path = ""`) is treated as unset rather than
/// as a path to the empty string, which would otherwise be reported as a
/// configured socket and trigger doomed connection attempts.
fn non_empty_socket_path(path: PathBuf) -> Option<PathBuf> {
