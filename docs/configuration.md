...
| `WISP_NO_ZOXIDE` | Disables zoxide only when set to a truthy value (`1`, `true`, `yes`, `on`); a falsy value (`0`, `false`, `no`, `off`) keeps zoxide enabled. Inverts the `zoxide.enabled` override. |
